  ...
  # 2. Quality control
{
  ...
  peaco_QC <- function(fsc, file_name) {
    ...
    fsc_peaco_qc = read.flowSet(path, truncate_max_range = FALSE, column.pattern = "Time", invert.pattern = TRUE) # Read the result stored in path as a flowSet
    ...
    ############################
      ...
      heatmap_cyto <- function(df_clust) {
        if ('cluster_id' %in% colnames(df_clust)){
          ...
          stop('Clusters  associated to each cells must be contained in column "cluster_id')
        }
        return (heat)
      }
      ...
      #plt_marker <- marker_expression(df_KNN,"FSC-H",df_umap)
