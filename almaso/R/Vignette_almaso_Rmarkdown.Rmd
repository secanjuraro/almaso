...
library(knitr)
knitr::opts_chunk$set(fig.width=10, fig.height=6, echo=TRUE, warning=FALSE, message=FALSE, cache=FALSE)
source("pipeline_functions.R", local = knitr::knit_global())
```
...
The functions used in this vignette are stored in the *pipeline_functions.R .*
...
To normalize data, we decided to treat the flow cytometry data as scRNA-Seq data. So we used TMM and logCPM scaling and normalization on the expression matrix.

```{r normalization, echo=TRUE}
## Attention NA
exp_mat_tmm = tmm(exp_mat_control, long = 1000, lc = 0, k = 0, refColumn = 1, logratioTrim = 0.3, sumTrim = 0.05, doWeighting = TRUE, Acutoff = -1e+10)

exp_mat_cpm = cpm(exp_mat_tmm)
```
...
```{r count_plot}
df_KNN %>% group_by(cluster_id) %>% summarise(total_count=n(), .groups = 'drop') %>% ggplot(aes(x = cluster_id, y = total_count)) + geom_bar(stat = "identity", color = 'Orange') 
