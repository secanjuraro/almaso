...
exp_matr_seurat <- as.data.frame(GetAssayData(object = seurat_object, slot = "counts"))
head(exp_matr_seurat)
...
############################
{
  ...
  clustering_cyto <- function(df_pca, expr_df, resolution) {
    ...
    louvain_clusters <- igraph::cluster_louvain(KNN_graph, resolution = resolution) # Implementation of the Louvain method to find clusters 
  ...
  df_KNN <- clustering_cyto(df_pca, expr_matrix, 0.5)
...

{
  ...
  marker_expression <- function(df_KNN,marker,df_umap){
    marker <- paste0(marker)
    marker_value <- df_KNN[[marker]]
    mid<-mean(marker_value)
    plot_marker <- ggplot(df_umap, aes(UMAP1,UMAP2, colour = as.numeric(marker_value))) +  geom_point() +  labs(x = "UMAP1",y = "UMAP2",subtitle = "UMAP plot ")+scale_color_gradient2(midpoint=mid, low="blue", mid="white", high="red", space ="Lab" )
